...
- **★★ 현 제출본 = v46** (`AIP_final.dll` + `Rule_forTraining.xml`). 트리는 v42 이후 무변경.
  백업 사슬: `AIP_final_PRE_v46_backup.dll`(=v45) / `_PRE_v45_`(=v44) / `_PRE_v44_`(=v42계열).
  ⚠ `AIP_final.dll`은 **git에 추적되지 않는다**(`.gitignore`). 백업 파일이 유일한 복구 수단이다.

## ★★ 현 제출본 = v46 (2026-08-21) — 스로틀 변화율 제한 해제
```cpp
float stepUse = 0.008f;                          // v45까지: 10Hz에서 0↔1에 **12.5초**
if (!Ablation::off("v46thr")) stepUse = 1.0f;    // v46: 한 틱에 전 구간
```
**메커니즘(vs yuno 같은 4시드, 사거리 안):** 우리 ≤1° 틱 **4→30(7.5배)**, 상대 ≤1° 틱 **99→46(절반)**.
- 우리: 평균 조준은 나빠지고(ATA 중앙 48.4→68.5°) **극단이 좋아진다**(최소 0.19→0.09°).
  사격 판정이 "한 틱이라도 1° 안"이라 **극단이 보상받는다.**
- 상대: 우리 속도가 예측 불가능해져 **상대 리드 예측이 빗나간다**(상대 ATA 중앙 112.9→95.1°).

**30시드 × 3상대: 50승32무8패 → 52승34무4패** (승 +2, 패 −4). 두 독립표본 모두 패 감소.
우리 추락 **0/180**. 제출본 등가 **45/45 시드일치**.
⚠ **준데미지만 보고 한 번 기각했었다** — 위 "가치 순서" 절 참조.
상세: `상대기체 공유파일/2026-08-21_v46_스로틀변화율해제_채택/README.md`

### v45 (v46의 토대, 2026-08-16) — 강하 클램프 해제
```cpp
diveSlope = dist*0.5 -> dist*3.0    // 강하각 상한 약 27° -> 해제. 하강나선 기하에 진입 가능
diveCap   = 650 -> 2000
```
하강나선은 지속 25.4°/s로 수평선회(11.3)의 2배 이상인데 그 기하에 못 들어가고 있었다.
대가(156°당 고도 1967m)를 **우리만 낼 수 있다** — 하한 우리 700 / jung·jh2 800 / yuno 1800 / TW 3000.
15시드×3상대 합계 +19.835 → +24.314, 준데미지 +21.7%, 추락 0/45.
⚠ v40 때 "조준만 내리고 벽은 그대로"라 기각됐던 것인데 **그 벽이 v40·v42에서 내려가 조건이 바뀌었다.**
되돌리려면 절제 플래그 `v45dive`.
