...
void UCPPBehaviorTree::init()
{
	...
	try
	{
		...
		*/
		...
		tree = Factory.createTreeFromFile("./Rule_v27.xml")   // 개발용. **제출 빌드 시 "./Rule_forTraining.xml"로 바꿔 AIP_final.dll 생성**;
...
StickValue UCPPBehaviorTree::Step(PlaneInfo MyInfo, int NumofOtherPlane, PlaneInfo* OthersInfo, Vector3& VP, float& Throttle)
{
	...
	{
		const double MAX_OFFBORESIGHT_DEG = 75.0;   // v28(89로 상향)은 no-op으로 판명 - 실전에서 off-boresight가 15~38도라 클램프 자체가 발동하지 않는다(clamped=0 실측). 원복.
