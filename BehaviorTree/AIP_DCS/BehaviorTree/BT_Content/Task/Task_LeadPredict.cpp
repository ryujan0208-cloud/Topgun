...
NodeStatus Action::Task_LeadPredict::tick()
{
	...
	// ★ v36: 260 -> 287. 260은 실측 검증된 적이 없는 값이었다("210~290의 중앙"이라는 근거뿐).
	//  [실측 2026-08-06 tools_diag/corner_speed.py — 뱅크82 수평선회, 총 각속도 기준]
	//    스로틀 0.65 -> 220m/s, 11.0deg/s, 반경 1144m  <- 반경 최소
	//    스로틀 0.80 -> 287m/s, 12.8deg/s, 반경 1281m  <- **선회율 최대 = 코너속도**
	//    스로틀 1.00 -> 329m/s, 11.0deg/s, 반경 1710m
	//  **선회율 최적(287)과 반경 최소(220)는 다른 속도다.** 260은 둘 중 어느 쪽도 아닌 중간값.
	//  [왜 선회율인가] onecircle 무승부 12판의 실체는 각도를 못 좁히는 것이다:
	//    사거리 안에 216초를 있는데 P1(ATA<=1도) 사격 0틱, 조준 최상위 400틱에서도
	//    수평오차 14.76도(수직은 1.88도로 이미 해결됨). 거리가 아니라 **각도 능력**이 병목이다.
	//    현재 우리 11.7deg/s vs 상대 11.9deg/s로 밀린다. 287이면 12.8로 뒤집힌다.
	//  ※ 상대 상수(255)에 맞추는 게 아니라 **우리 기체의 실측 최적점**이다(과적합 아님).
	const double CORNER = 287.0;
