...
ControlValue GetStick(oPlaneData& MyData, float VP_X, float VP_Y, float VP_Z)
{
    ...
    if(BT_item != BTList.end())
    {
        // 2026-08-15: removed the LLAtoCartesian call that used to be here.
        //  oPlaneData.Location is ALREADY Cartesian (meters): ChangeData() converts it,
        //  and the match server sends Cartesian directly. Converting again read metres
        //  as degrees, so the position was garbage while VP stayed in metres -- the two
        //  arguments below were in different frames and the aircraft flew blind.
        //  Measured: invariance test (same relative geometry, different absolute position)
        //  gave different stick output before this change, identical after.
        //  The organiser said on 2026-05-25 this line "should have been deleted".
        //  It was deleted in Step() but survived here. Same bug, two places.
...
oPlaneData ChangeData(int ID, int Team, float HP, int OperType, oNavigationData& NaviData)
{
    ...
    // 2026-08-15: this is now the ONLY place where LLA -> Cartesian happens.
    //  oNavigationData is LLA by definition (scaled integers), so this can never
    //  double-convert. Every other entry point (Step, StepWithPlaneData, GetStick)
    //  now takes oPlaneData.Location as Cartesian metres and touches it no further --
    //  which is also what the match server sends. One rule, no detection, no branch.
    //  Before this, conversion lived in Step() and GetStick(); the server path fed
    //  Cartesian into both and they read metres as degrees (distance 1.05e+08 m in a
    //  real match on 2026-08-15). Fixing one and missing the other is the failure this
    //  layout removes: there is no longer a per-entry-point decision to get wrong.
    BT_Geometry::Vector3 Cart = LLAtoCartesian(LLA, Vector3(37.91455691666666, 128.18188127777776, 0));
    PlaneData.LocationX = Cart.X;
    PlaneData.LocationY = Cart.Y;
    PlaneData.LocationZ = Cart.Z;
