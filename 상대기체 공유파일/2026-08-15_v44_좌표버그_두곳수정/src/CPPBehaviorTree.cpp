...
void UCPPBehaviorTree::init()
{
	...
	try
	{
		...
		*/
		...
		// 연구용: 실험마다 Rule 파일을 바꿔치기하면 원본을 덮어써 사고가 난다
		// (오늘 Rule_mine.xml 충돌로 kwon 배치가 이틀간 죽어 있었다).
		// 환경변수로 고르게 하고, 미설정이면 제출본과 동일한 현역 Rule을 읽는다.
		// v40: Rule_v32.xml -> Rule_v40.xml (DECO_AltitudeCheck MinAlt 1800 -> 1000).
		// ★ 조준점 하한(Task_LeadPredict)과 이 MinAlt는 **반드시 같이** 낮춘다.
		//   하나만 낮추면 내려가려다 벽에 부딪혀 에너지만 잃는다(divefree 기각 사유).
		const char* ruleFile = "./Rule_v40.xml";
#if defined(_MSC_VER)
		static char __rulebuf[128]; size_t __rn = 0;
		if (getenv_s(&__rn, __rulebuf, sizeof(__rulebuf), "TOPGUN_RULE") == 0 && __rn > 1)
			ruleFile = __rulebuf;
#else
		if (const char* e = std::getenv("TOPGUN_RULE")) if (e[0]) ruleFile = e;
#endif
		tree = Factory.createTreeFromFile(ruleFile);
