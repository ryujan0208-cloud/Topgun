...
def _track_path(case_dir: Path, stamp: str, role: str) -> Path:
    suffix = (
        "ownship_(F-16)[Blue].csv" if role == "ownship" else "target_(F-16)[Red].csv"
    )
    return case_dir / f"{stamp}_{suffix}"


def _index_at_or_before(
    points: Sequence[state_dataset.TrackPoint], time_s: float
) -> int:
    if not points:
...
def _load_case(
    ...
    dict[str, object], list[state_dataset.TrackPoint], list[state_dataset.TrackPoint]
]:
...
def iter_rows(
    campaign_dirs: Iterable[Path], horizons_s: Sequence[float]
    ...
    for campaign_dir in campaign_dirs:
        ...
        for case_dir in sorted(
            path for path in campaign_dir.iterdir() if path.is_dir()
        ):
            if (
                case_dir.name == "baseline"
                or not (case_dir / "case_manifest.json").is_file()
            ):
                continue
...
def main() -> int:
    ...
    parser.add_argument(
        "--horizons", type=_parse_horizons, default=_parse_horizons("3,5,10,20")
    )
    ...
    print(
        f"wrote {len(rows)} fork outcomes, prefix_failures={prefix_failures}: {args.output}"
    )
