...
LOS_FEATURES = (
    ...
    "target_los_az_deg",
    "target_los_el_deg",
    "target_lift_to_los_deg",
    "own_los_az_rate_dps",
    "own_los_el_rate_dps",
    "target_los_az_rate_dps",
    "target_los_el_rate_dps",
)
...
def _body_axes(point: TrackPoint):
    yaw = math.radians(point.yaw)
    pitch = math.radians(point.pitch)
    roll = math.radians(point.roll)
    forward = _forward(point)
    level_right = (math.cos(yaw), -math.sin(yaw), 0.0)
    level_lift = (
    ...
    right = tuple(
        r * math.cos(roll) - u * math.sin(roll) for r, u in zip(level_right, level_lift)
    )
    lift = tuple(
        r * math.sin(roll) + u * math.cos(roll) for r, u in zip(level_right, level_lift)
    )
    return forward, right, lift


def _relative_enu(origin: TrackPoint, target: TrackPoint) -> tuple[float, float, float]:
...
def _signed_los_angles(point: TrackPoint, los: Sequence[float]):
    forward_axis, right_axis, lift_axis = _body_axes(point)
    forward = _dot(los, forward_axis)
    right = _dot(los, right_axis)
    lift = _dot(los, lift_axis)
    azimuth = math.degrees(math.atan2(right, forward))
    elevation = math.degrees(math.atan2(lift, math.hypot(forward, right)))
    lift_separation = _angle_deg(lift_axis, los)
    return azimuth, elevation, lift_separation
...
def episode_ranges(
    ...
    for index in range(1, count):
        time_rewound = (
            own[index].time <= own[index - 1].time
            or target[index].time <= target[index - 1].time
        )
        jumped = (
...
def _los_features(own, target, index, window, start_index):
    los = _relative_enu(own[index], target[index])
    reverse_los = tuple(-value for value in los)
    own_angles = _signed_los_angles(own[index], los)
    target_angles = _signed_los_angles(target[index], reverse_los)
    rates = [0.0, 0.0, 0.0, 0.0]
    previous = max(start_index, index - window)
    dt = own[index].time - own[previous].time
    if dt > 1e-6:
        previous_los = _relative_enu(own[previous], target[previous])
        previous_reverse = tuple(-value for value in previous_los)
        previous_own = _signed_los_angles(own[previous], previous_los)
        previous_target = _signed_los_angles(target[previous], previous_reverse)
        rates = [
            _wrap_delta_deg(current, old) / dt
            for current, old in zip(
                own_angles[:2] + target_angles[:2],
                previous_own[:2] + previous_target[:2],
            )
        ]
    return {
        "own_los_az_deg": own_angles[0],
        "own_los_el_deg": own_angles[1],
        "own_lift_to_los_deg": own_angles[2],
        "target_los_az_deg": target_angles[0],
        "target_los_el_deg": target_angles[1],
        "target_lift_to_los_deg": target_angles[2],
        "own_los_az_rate_dps": rates[0],
        "own_los_el_rate_dps": rates[1],
        "target_los_az_rate_dps": rates[2],
        "target_los_el_rate_dps": rates[3],
    }
...
def _base_features(
    ...
    los_features = _los_features(own, target, index, derivative_window, start_index)
    ...
    return {
        **los_features,
        ...
        "target_turn_rate_dps": _turn_rate(
            target, index, derivative_window, start_index
        ),
        "energy_height_delta_m": (
            opponent.alt
            + opponent_speed * opponent_speed / (2.0 * G)
            - my.alt
...
def _future_index(
    times: Sequence[float], index: int, horizon_s: float, end: int
) -> int | None:
...
def iter_rows(
    ...
    for episode, (start, end) in enumerate(
        episode_ranges(own, target, jump_threshold_m)
    ):
        ...
        for index in range(start, end):
            ...
            for horizon in horizons_s:
                ...
                if future is None:
                    ...
                    continue
                future_features = _base_features(
                    own, target, future, derivative_window, start
                )
...
def _parse_horizons(text: str) -> list[float]:
    ...
    if not horizons or horizons[0] <= 0.0:
        raise argparse.ArgumentTypeError(
            "horizons must be positive comma-separated seconds"
        )
    return horizons
...
def main() -> int:
    ...
    parser.add_argument(
        "--horizons", type=_parse_horizons, default=_parse_horizons("2,5,10,20")
    )
