...
NodeStatus Action::Task_LeadPredict::tick()
{
	...
	float target;
	if (dvTarget > 900.0) {
		// ★ v39: 사거리 밖이라도 **속도 상한**을 둔다. 무제한 풀스로틀이 진짜 병목이었다.
		//  [실측 2026-08-06 / 직진 상대 200초]
		//    스로틀 0.98~0.99로 200초 -> 속도 중앙 **496m/s**
		//    요 변화율 0.6~5도/s -> 선회반경 r=v/w ≈ **28,000m**
		//    t=10s 거리 1322m -> t=20s **5744m** (10초에 5.7km 이탈), 복귀에 160초
		//  [선회율 실측표 tools_diag/corner_speed.py]
		//    287m/s -> 12.8도/s (최대)  /  329m/s -> 11.0도/s (14% 저하)  /  500m/s -> 사실상 못 돔
		//    -> **330m/s를 넘으면 접근은 조금 빨라지고 선회 능력은 급격히 무너진다.**
		//  [v38(코너속도 거리게이트 제거)과의 차이 — 그건 기각됐다]
		//    v38은 "돌아야 할 때 감속"이라 `ataDeg>12` 조건이 필요했고,
		//    직선 추격 중엔 ATA가 작아 **가속하는 동안 안 걸렸다.**
		//    이건 **ATA와 무관한 순수 속도 상한**이라 가속 자체를 막는다.
		//  ※ v5 교훈('원거리 감속은 에너지 손실')과 충돌하지만, 330m/s는 코너속도(287)보다
		//    여전히 빠르다. 에너지를 버리는 게 아니라 **못 쓰는 초과분만** 잘라낸다.
		const double APPROACH_CAP = 330.0;
		if (mySpd > APPROACH_CAP) {
			double over = (mySpd - APPROACH_CAP) / 100.0;   // 0~1
			if (over > 1.0) over = 1.0;
			double u = 1.0 - over * 0.55;                   // 430m/s에서 0.45까지
			if (u < 0.45) u = 0.45;
			target = (float)u;
		} else {
			target = 1.0f;                  // 상한 아래면 기존대로 풀스로틀
		}
	} else {
