...
class PrefixForkTest(unittest.TestCase):
    ...
    def test_ned_position_is_converted_to_neu(self):
        ...
        np.testing.assert_allclose(subject.neu_position(state), [100.0, 200.0, 3_000.0])

    def test_vector3_rejects_wrong_shape(self):
        self.assertTrue(all(np.isnan(subject._vector3([1.0, 2.0]))))
        self.assertEqual(subject._vector3([1.0, 2.0, 3.0]), (1.0, 2.0, 3.0))
    ...
    def test_candidate_vp_has_interpretable_offsets(self):
        ...
        np.testing.assert_allclose(
            subject.candidate_vp("bt_up", position, velocity, 2.0, 10.0),
            [0.0, 0.0, 10.0],
        )
        np.testing.assert_allclose(
            subject.candidate_vp("bt_down", position, velocity, 2.0, 10.0),
            [0.0, 0.0, -10.0],
        )
