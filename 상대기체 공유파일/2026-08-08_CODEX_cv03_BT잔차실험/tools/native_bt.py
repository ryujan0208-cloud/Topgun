...
class AIPilot:
    def __init__(self, filename: str = "AIP_DCS_ownship.dll"):
        ...
        self.AIPilotDLL.ChangeData.argtypes = [
            ct.c_int,
            ct.c_int,
            ct.c_float,
            ct.c_int,
            ct.POINTER(J_NavigationData),
        ]
        self.AIPilotDLL.ChangeData.restype = OPlaneData

        self.AIPilotDLL.Step.argtypes = [
        ...
        try:
            self._step_with_vp_residual = self.AIPilotDLL.StepWithVPResidual
        except AttributeError:
            self._step_with_vp_residual = None
        else:
            self._step_with_vp_residual.argtypes = (
                list(self.AIPilotDLL.Step.argtypes) + [ct.c_float] * 3
            )
            self._step_with_vp_residual.restype = ControlValue
    ...
    def CreateBehaviorTree(self, my_id, my_force_id):
        ...
        self._debug_log(f"after CreateBehaviorTree my_id={my_id} force={my_force_id}")

    def _invoke_vp_export(self, function, *args):
        if function is None:
            raise RuntimeError("DLL does not provide residual VP step")
        my_id, my_force_id, tgt_id, tgt_force_id, my_navi, tgt_navi, vp_xyz = args
        b_lockon = False
        b_flare = ct.c_bool()
        b_launch_missile = ct.c_bool()
        my_opd = self.AIPilotDLL.ChangeData(
            my_id, my_force_id, 100.0, 0, ct.POINTER(J_NavigationData)(my_navi)
        )
        tgt_opd = self.AIPilotDLL.ChangeData(
            tgt_id, tgt_force_id, 100.0, 0, ct.POINTER(J_NavigationData)(tgt_navi)
        )
    ...
    def StepWithVPResidual(self, *args):
        return self._invoke_vp_export(self._step_with_vp_residual, *args)

    @staticmethod
    def BuildPlaneData(
        location_xyz, rotation_rpy, speed: float, team: int
    ) -> OPlaneData:
    ...
    def GetVP(self, my_id, my_force_id, my_navi):
        try:
            ...
            raise OSError(
                f"BT GetVP failed dll={self.dll_path} my_id={my_id} force={my_force_id}"
            ) from exc
