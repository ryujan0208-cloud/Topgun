...
def candidate_vp(
    ...
    if candidate == "bt_up":
        return np.array([0.0, 0.0, vertical_offset_m])
    if candidate == "bt_down":
        return np.array([0.0, 0.0, -vertical_offset_m])
    if candidate == "pure":
...
class VPForkProvider(BTActionProvider):
    ...
    def compute_action(self, context: ActionContext) -> ActionResult:
        ...
        vp_step = (
            self.ai_pilot.StepWithVPResidual
            if self.candidate.startswith("bt_")
            else self.ai_pilot.StepWithVPOverride
        )
        control_action = vp_step(
            model.fighterID,
            model._forceSide,
        ...
        applied_vp = (
            raw_vp_array.astype(np.float64) + override_vp
            if self.candidate.startswith("bt_")
            else override_vp
        )
        override_vp = applied_vp
        action = clip_action(
            [
