...
extern "C"
{
    ...
    */
	...
	__declspec(dllexport) ControlValue StepWithVPResidual(oPlaneData& MyData, int NumOfOthers, oPlaneData* Others, bool isLockedOn, bool& MSL_Lunch_Possible, bool& Flare_Lunch_Possible, float VP_X, float VP_Y, float VP_Z);
...
static ControlValue StepInternal(oPlaneData& MyData, int NumOfOthers, oPlaneData* Others, bool isLockedOn, bool & MSL_Lunch_Possible, bool & Flare_Lunch_Possible, const Vector3* VPOverride, const Vector3* VPResidual)
{
    ControlValue value;
	value.RollCMD = 0;
	value.PitchCMD = 0;
   ...
   // cout << "target:" << others[0].Team  << " lat:" << others[0].Location.X << " lon:" << others[0].Location.Y << " alt:" << others[0].Location.Z << endl;
    ...
    {
        StickValue v = BT_item->second->Step(my, NumOfOthers, others, VP, throttle, VPOverride, VPResidual);
...
{
	return StepInternal(MyData, NumOfOthers, Others, isLockedOn, MSL_Lunch_Possible, Flare_Lunch_Possible, nullptr, nullptr);
}
...
ControlValue StepWithVPOverride(oPlaneData& MyData, int NumOfOthers, oPlaneData* Others, bool isLockedOn, bool & MSL_Lunch_Possible, bool & Flare_Lunch_Possible, float VP_X, float VP_Y, float VP_Z)
{
	...
	return StepInternal(MyData, NumOfOthers, Others, isLockedOn, MSL_Lunch_Possible, Flare_Lunch_Possible, &OverrideVP, nullptr);
}

ControlValue StepWithVPResidual(oPlaneData& MyData, int NumOfOthers, oPlaneData* Others, bool isLockedOn, bool & MSL_Lunch_Possible, bool & Flare_Lunch_Possible, float VP_X, float VP_Y, float VP_Z)
{
	Vector3 ResidualVP(VP_X, VP_Y, VP_Z);
	return StepInternal(MyData, NumOfOthers, Others, isLockedOn, MSL_Lunch_Possible, Flare_Lunch_Possible, nullptr, &ResidualVP);
}
