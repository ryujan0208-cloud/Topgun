...
NodeStatus Action::Task_LeadPredict::tick()
{
	...
	// ★★ v46 채택 (2026-08-21): 스로틀 변화율 제한 해제를 **기본값**으로.
	//  되돌리려면 절제 플래그 "v46thr" 로 v45 값(0.008)으로 복귀한다.
	//  [규명된 메커니즘 — vs yuno 같은 4시드, 사거리 안 득점 기회]
	//                우리<=1도   우리<=3도   상대<=1도   상대<=3도
	//      base          4         110         99         156
	//      해제         30         122         46          75
	//    **우리 득점 기회 7.5배, 상대 득점 기회 절반.**
	//    준데미지 0.215->0.447(2배), 받은 0.664->0.130(1/5)이 이 수치에서 나온다.
	//  [왜]  ① 우리: 평균 조준은 나빠진다(사거리 안 ATA 중앙 48.4->68.5도). 대신 **극단이
	//        좋아진다**(최소 0.19->0.09도). 사격 판정은 평균이 아니라 "한 틱이라도 1도 안"
	//        이므로 극단이 보상받는다.
	//        ② 상대: 우리 속도가 예측 불가능해져 **상대의 리드 예측이 빗나간다.**
	//        상대 ATA 중앙 112.9->95.1도, <=1도 틱 99->46.
	//  [성적] 30시드 x 3상대: 50승32무8패 -> **52승34무4패** (승 +2, 패 **-4**).
	//    두 독립 표본 모두 패가 줄었다(5->3, 3->1). yuno 순이득이 둘 다 음->양으로 바뀐다.
	//    우리 추락 0/180.
	//  [판정 기준을 고친 이유] 처음엔 "어느 상대든 준데미지가 줄면 즉시 기각"으로 봤다.
	//    그 기준은 v25의 실패 양식(준데미지와 **승수가 함께** 붕괴)을 막으려던 것인데,
	//    이건 정반대다 — 준데미지는 줄지만 승수가 는다.
	//    규정이 200초 HP 비교이므로 **받은 데미지가 0인 상대에서 준데미지 5% 감소는
	//    승패에 영향이 없다.** 가치 순서는 승리 > 준 데미지 > 무피격이다.
	if (!Ablation::off("v46thr")) stepUse = 1.0f;      // 한 틱에 전 구간 이동 (기본)
