...
private:
	...
	// ★ 2026-08-15: 입력 좌표 규약. 0=미정 1=LLA(위도,경도,고도m) 2=직교 미터(N,E,U)
	//  [왜] Step()은 입력을 무조건 LLA로 보고 LLAtoCartesian을 건다. 로컬 경로
	//    (ChangeData)는 LLA를 주지만 **대회 서버 경로는 직교 미터를 준다.**
	//    그대로 두면 608.5를 "위도 608.5도"로 읽어 거리가 1.05e+08 m가 되고
	//    기체가 200초 내내 장님으로 난다(2026-08-15 실서버 실전에서 확인).
	//  [고정] 한 번 정해지면 트리 수명 동안 유지한다. 매 틱 재판별하면
	//    기체가 직교 원점 근처를 지날 때 판정이 흔들린다(실측: 적기가 원점 7m까지 온다).
	int  CoordFrameMode;
	int  DecideCoordFrame(const Vector3& mine, int nOthers, const PlaneInfo* others) const;
