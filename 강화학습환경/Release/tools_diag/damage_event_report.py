...
def summarize(
    ...
    for kind in ("dealt", "taken"):
        ...
        largest_event = max(selected, key=lambda row: float(row["damage"]), default=None)
        largest_episode = max(episode_damage.items(), key=lambda item: item[1], default=None)
        horizon_reports: dict[str, object] = {}
        for horizon in horizons_s:
            prefix = _horizon_key(horizon)
            available = [row for row in selected if int(row[f"{prefix}_available"]) == 1]
            horizon_reports[f"{horizon:g}"] = {
                "available": len(available),
                "features": {
                    feature: {
                        "p10": state_report.quantile(
                            [float(row[f"{prefix}_{feature}"]) for row in available], 0.10
                        ),
                        "p50": state_report.quantile(
                            [float(row[f"{prefix}_{feature}"]) for row in available], 0.50
                        ),
                        "p90": state_report.quantile(
                            [float(row[f"{prefix}_{feature}"]) for row in available], 0.90
                        ),
                    }
        ...
        kinds[kind] = {
            ...
            "largest_event_id": None if largest_event is None else largest_event["event_id"],
            "largest_event_damage": None if largest_event is None else float(largest_event["damage"]),
            "largest_event_share": (
                None
            ...
            "largest_episode_damage": None if largest_episode is None else largest_episode[1],
