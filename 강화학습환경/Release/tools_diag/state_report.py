...
def print_report(label: str, stamp: str, report: dict[str, object]) -> None:
    print(f"[{label}] stamp={stamp} rows={report['total_rows']} episodes={report['episodes']} horizon={report['horizon_s']}s")
    buckets = report["buckets"]
    assert isinstance(buckets, dict)
    for name in ("all_available", "future_dealt", "future_taken", "future_positive_net", "future_negative_net"):
        bucket = buckets[name]
...
def main() -> int:
    ...
    own = state_dataset.load_track(args.logdir / f"{args.stamp}_ownship_(F-16)[Blue].csv")
    target = state_dataset.load_track(args.logdir / f"{args.stamp}_target_(F-16)[Red].csv")
