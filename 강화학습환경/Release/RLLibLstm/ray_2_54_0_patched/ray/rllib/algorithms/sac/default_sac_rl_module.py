...
class DefaultSACRLModule(RLModule, InferenceOnlyAPI, TargetNetworkAPI, QNetAPI):
    ...
    # Return the actor encoder state so RLlib's ConnectorV2 stack treats SAC as
    # stateful. This makes EnvRunner store STATE_OUT in episodes and makes the
    # learner replay path request sequence batches via model_config["max_seq_len"].
    # If dogfight_lstm_scope=actor_critic, Q recurrent state is still not exposed
    # here; the torch Q helper zero-initializes critic state inside learner forward.
    @override(RLModule)
    def get_initial_state(self) -> dict:
        if hasattr(self, "pi_encoder") and hasattr(
            self.pi_encoder, "get_initial_state"
        ):
